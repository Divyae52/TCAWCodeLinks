...
def guessWord(availableList, wordList):
    ...
    bestGuess = None
    bestScore = float('inf')  # Lower is better

    for guess in wordList:
        pCounts = {}

        for target in availableList:
            pattern = tuple(scoreGuess(guess, target))  # Convert list to tuple so it can be a dict key
            if pattern not in pCounts:
                pCounts[pattern] = 0
            pCounts[pattern] += 1

        # Worst case: what's the largest group of possible words remaining?
        worstCase = mean(pCounts.values()) if pCounts else 0
